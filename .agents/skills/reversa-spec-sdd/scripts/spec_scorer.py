...
class DimensionScore:
    name: str
    weight: float          # relative weight (sums to 1.0)
...
def section_content(text: str, section_pattern: str) -> str:
    """Extract the section's content up to the next section at the same level."""
...
def count_rf_items(text: str) -> int:
    return count_pattern(text, r'\bFR-\d+\b')
...
def has_vague_terms(text: str) -> list[str]:
    vague = ["quickly", "soon", "shortly", "some", "many",
             "efficiently", "intuitively", "easy to use",
             "user-friendly", "performant", "look nice", "be fast",
             "be good", "easily", "fast", "nice",
             "rapidamente", "logo", "alguns", "muitos", "fácil de usar"]
    found = []
    for term in vague:
...
def has_contradictions_signal(text: str) -> bool:
    # Simple heuristic: the presence of "but" / "however" / "although" after a requirement
    return bool(re.search(r'FR-\d+.*?\b(but|however|although|mas|porém|entretanto)\b', text,
...
def score_completeness(text: str) -> DimensionScore:
    ...
    required_sections = [
        (r'^#{1,2}\s+1[\.\s]+(Summary|Resum)', "Section 1 (Summary)"),
        (r'^#{1,2}\s+2[\.\s]+(Context|Contexto)', "Section 2 (Context)"),
        (r'^#{1,2}\s+3[\.\s]+Goals', "Section 3 (Goals)"),
        (r'^#{1,2}\s+4[\.\s]+Non.Goals', "Section 4 (Non-Goals)"),
        (r'^#{1,2}\s+5[\.\s]+(Users|Usuári)', "Section 5 (Users)"),
        (r'^#{1,2}\s+6[\.\s]+(Functional Requirements|Requirements|Requisitos)', "Section 6 (Requirements)"),
    ]
    present = 0
    for pattern, name in required_sections:
        if has_section(text, pattern):
            ...
            if has_content(content, 8):
                present += 1
                dim.positives.append(f"✅ {name} present and filled in")
            else:
    ...
    if rf_count >= 5:
        score += 8
        dim.positives.append(f"✅ {rf_count} numbered FR requirements")
    elif rf_count >= 3:
        score += 5
        dim.issues.append(f"⚠️ Only {rf_count} FR requirements — a minimum of 5 is recommended")
    elif rf_count > 0:
        score += 2
        dim.issues.append(f"❌ Too few FR requirements ({rf_count}) — the spec is incomplete")
    else:
        dim.issues.append("❌ No numbered FR requirements — impossible to trace")
    ...
    if placeholders == 0:
        ...
        dim.issues.append(f"❌ {placeholders} unfilled placeholder(s) — the spec is incomplete")
...
def score_testability(text: str) -> DimensionScore:
    ...
    # Concrete verbs — heuristic: the absence of vague terms in requirements
    vague = has_vague_terms(text)
    if not vague:
        score += 10
        dim.positives.append("✅ No vague terms in the requirements")
    else:
    ...
    has_happy_path = has_section(text, r'Main flow|Fluxo Principal|Happy Path|6\.2')
    if has_happy_path:
        # Check that it has at least 3 numbered steps
        happy_content = section_content(text, r'Main flow|Fluxo Principal|Happy Path|6\.2')
    ...
    if has_numeric_metric(goals_content) or has_numeric_metric(text[:2000]):
        ...
        dim.issues.append("⚠️ Success metrics with no numeric values — hard to validate")
...
def score_clarity(text: str) -> DimensionScore:
    ...
    ambiguities_hidden = count_pattern(text, r'\?.*\?')  # multiple question marks — a sign of doubt
    if open_questions > 0:
        score += 6
        dim.positives.append(f"✅ {open_questions} open question(s) flagged explicitly")
    elif ambiguities_hidden > 3:
        dim.issues.append("⚠️ Possible unflagged ambiguities (use ⚠️ OPEN: or section 14)")

    # A clear subject in the requirements
    rf_section = section_content(text, r'^#{1,2}\s+6[\.\s]+(Functional Requirements|Requirements|Requisitos)')
    ...
    if rf_count > 0 and subjects >= rf_count * 0.5:
        score += 6
        dim.positives.append("✅ Requirements with a clear subject (system/user)")
    elif rf_count > 0:
    ...
    if has_contradictions_signal(text):
        score = max(0, score - 5)
        dim.issues.append("⚠️ Possible contradiction between requirements — review it")

    # Technical language with no definition (heuristic)
    vague = has_vague_terms(text)
    if not vague:
        score += 8
...
def score_scope(text: str) -> DimensionScore:
    ...
    if ng_count >= 3 and has_content(ng_section, 15):
        score += 7
        dim.positives.append(f"✅ Clear, specific non-goals ({ng_count} items)")
    elif ng_count >= 1:
        score += 4
        dim.issues.append("⚠️ Non-goals present but they could be more specific")
    else:
        dim.issues.append("❌ Non-goals missing")

    # Mapped dependencies
    has_deps = has_section(text, r'10[\.\s]+Integra|Dependencies|Dependências')
    if has_deps:
        deps_content = section_content(text, r'10[\.\s]+Integra|Dependencies|Dependências')
...
def score_edge_cases(text: str) -> DimensionScore:
    ...
    # Edge cases with a defined behavior
    has_behavior = count_pattern(ec_section, r'\|[^|]{5,}')
    if has_behavior >= ec_count * 2:  # at least a trigger + a behavior
        score += 3
        dim.positives.append("✅ Edge cases with a defined expected behavior")
    else:
        dim.issues.append("⚠️ Edge cases with no defined behavior — they are just questions, not a spec")

    # Coverage of external failures
    covers_external = bool(re.search(
        r'(timeout|unavailable|down|failure|error\s+\d{3}|retry|fallback|indisponível|falha)',
    ...
    if covers_external:
        ...
        dim.issues.append("⚠️ Does not cover external system failures (timeouts, outages)")
...
def classify(score: float) -> str:
    ...
    return "❌ Insufficient — back to the interview/draft"
...
def main():
    ...
    parser.add_argument("--spec", required=True, help="Path to the spec's .md file")
    ...
    # Non-zero exit code if the score is < 65 (for CI/CD use)
