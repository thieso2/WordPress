...
Automatically detects the format, the encoding and the structure of the data.
Output: JSON ready to be embedded in the Highcharts options.
...
def parse_number(value: str) -> float | None:
    """Convert a string to a number, handling both BR and US formats."""
...
def parse_csv(filepath: str, encoding: str = 'utf-8', delimiter: str = None) -> dict:
    """Parse CSV into the Highcharts format."""
    import csv
    ...
    if len(rows) < 2:
        return {"error": "File with fewer than 2 rows"}

    headers = [h.strip() for h in rows[0]]
    data_rows = rows[1:]

    # The first column = the categories, the rest = the series
...
def parse_json_data(filepath: str) -> dict:
    """Parse JSON into the Highcharts format."""
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)

    # If it is already in the Highcharts format, return it directly
...
def parse_excel(filepath: str, sheet: str = None) -> dict:
    """Parse Excel into the Highcharts format."""
    ...
    if len(rows) < 2:
        return {"error": "Spreadsheet with fewer than 2 rows"}
...
def main():
    parser = argparse.ArgumentParser(description="Parse data into the Highcharts format")
    ...
    parser.add_argument("--sheet", default=None, help="The Excel sheet's name")
    parser.add_argument("--output", "-o", help="Save the result to a file")
