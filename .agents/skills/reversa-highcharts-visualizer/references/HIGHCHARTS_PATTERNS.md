...
A reference of tested code patterns for generating professional Highcharts charts.
...
<head>
    ...
    <!-- Extra modules as needed (see the table in SKILL.md) -->
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* For multiple charts (a dashboard) */
        .chart-grid {
            display: grid;
...
Apply them before creating any chart. Adjust the `lang` block to the installation's
`doc_language` when it is not English:
...
```javascript
// The default Highcharts palette (good for most cases)
// It is the default, so it does not need to be set
...
## Formatted tooltips (useful patterns)
...
// A shared tooltip (multiple series)
tooltip: {
    shared: true,
    crosshairs: true,
    borderRadius: 8,
...
// A staggered animation (each series with a delay)
plotOptions: {
    series: {
        animation: { duration: 1000 },
        // each point appears with a delay
...
## Large datasets (the Boost module)
...
chart: {
    events: {
        load: function() {
            // Run once the chart has rendered
            console.log('Chart loaded');
        },
        redraw: function() {
            // After a resize or an update
        }
    }
},
plotOptions: {
