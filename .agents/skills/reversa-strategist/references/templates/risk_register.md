---
schemaVersion: 1
generatedAt: <ISO-8601>
reversa:
  version: "x.y.z"
kind: risk_register
producedBy: strategist
hash: "sha256:<hash of the body below the front-matter>"
---

# Risk Register

> Register of migration risks with probability, impact, mitigation and owner.

## Risks

### RISK-001
- **Description**: <text>
- **Category**: technical | operational | organizational | regulatory | financial
- **Probability**: low | medium | high
- **Impact**: low | medium | high | critical
- **Combined severity**: <derived from the two above>
- **Trigger / warning signal**: <what would indicate the risk is materializing>
- **Mitigation**: <concrete actions>
- **Contingency plan**: <if mitigation fails>
- **Owner**: <name / role>
- **Status**: open | mitigating | accepted | closed

<repeat per risk>

## Summary by severity

| Severity | Count | IDs |
|---|---|---|
| Critical | | |
| High | | |
| Medium | | |
| Low | | |

## Risks related to the target paradigm

> Dedicated subsection for when there is a paradigm change. List only risks whose direct origin is the gap recorded in `paradigm_decision.md`.

- <RISK-XXX: description>
