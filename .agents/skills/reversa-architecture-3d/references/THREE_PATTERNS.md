...
A quick reference for the setup, materials and techniques common to every mode of the skill. Three.js v0.158+, ESM via CDN.
...
```javascript
// A light ambient fill so the shadows are not completely black
const ambient = new THREE.AmbientLight(0xffffff, 0.35);
scene.add(ambient);

// Hemisphere: sky vs. ground, giving natural depth
...
When there are more than 200 elements of the same type (Code City buildings, dep-graph nodes), use an `InstancedMesh` instead of a loop with `add()`.
...
Inside `tick()`, call `labelRenderer.render(scene, camera)` along with the main renderer.
...
sliders.forEach((slider) => {
    slider.addEventListener("input", (e) => {
        ...
        applyParam(param, value); // a mode-specific function
...
| An InstancedMesh of cubes | 5,000 | Apply folder grouping |
| Lines (LineSegments) | 10,000 segments | Use Line2 (fat lines) or group them |
| Sprites/CSS2D labels | 100 visible | Show them only on hover or proximity |
| Textured polygons | 50,000 tris | Reduce the LOD or disable shadows |
