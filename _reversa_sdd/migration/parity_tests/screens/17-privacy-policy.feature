...
# spec-id: PT-S017
# traceability:
#   screen: web.privacy_policy (SCR-0143)
#   legacy_origin: wp-includes/template-loader.php -> privacy_policy
...
Feature: Visual parity for web.privacy_policy
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.privacy_policy" taken from the oracle
    When the target renders the route "/privacy-policy/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/privacy-policy/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.privacy_policy" is captured twice
