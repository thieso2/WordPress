...
# spec-id: PT-S001
# traceability:
#   screen: web.index (SCR-0127)
#   legacy_origin: wp-includes/template-loader.php -> index
...
Feature: Visual parity for web.index
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.index" taken from the oracle
  ...
  Scenario: The capture is deterministic
    ...
    When "web.index" is captured twice
