...
# spec-id: PT-S009
# traceability:
#   screen: web.tag (SCR-0135)
#   legacy_origin: wp-includes/template-loader.php -> tag
...
Feature: Visual parity for web.tag
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.tag" taken from the oracle
    When the target renders the route "/tag/sample/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/tag/sample/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.tag" is captured twice
