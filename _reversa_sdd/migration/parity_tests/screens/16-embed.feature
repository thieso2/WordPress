...
# spec-id: PT-S016
# traceability:
#   screen: web.embed (SCR-0142)
#   legacy_origin: wp-includes/template-loader.php -> embed
...
Feature: Visual parity for web.embed
  As the system
  ...
  # The embed template emits an iframe wrapper; the inner document is captured too.
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.embed" taken from the oracle
    When the target renders the route "/hello-world/embed/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/hello-world/embed/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.embed" is captured twice
