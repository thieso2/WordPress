...
# spec-id: PT-S014
# traceability:
#   screen: web.not_found_404 (SCR-0140)
#   legacy_origin: wp-includes/template-loader.php -> not_found_404
...
Feature: Visual parity for web.not_found_404
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.not_found_404" taken from the oracle
    When the target renders the route "/definitely-not-a-real-url/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/definitely-not-a-real-url/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.not_found_404" is captured twice
