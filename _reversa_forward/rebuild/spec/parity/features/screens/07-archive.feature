...
# spec-id: PT-S007
# traceability:
#   screen: web.archive (SCR-0133)
#   legacy_origin: wp-includes/template-loader.php -> archive
...
Feature: Visual parity for web.archive
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.archive" taken from the oracle
    When the target renders the route "/post/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/post/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.archive" is captured twice
