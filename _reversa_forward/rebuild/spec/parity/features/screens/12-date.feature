...
# spec-id: PT-S012
# traceability:
#   screen: web.date (SCR-0138)
#   legacy_origin: wp-includes/template-loader.php -> date
...
Feature: Visual parity for web.date
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.date" taken from the oracle
    When the target renders the route "/2026/01/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/2026/01/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.date" is captured twice
