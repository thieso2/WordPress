...
# spec-id: PT-S018
# traceability:
#   screen: web.comments (SCR-0144)
#   legacy_origin: wp-includes/template-loader.php -> comments
...
Feature: Visual parity for web.comments
  As the system
  ...
  # A PARTIAL, not a route. Capture within web.single and extract the region.
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.comments" taken from the oracle
    When the target renders the route "/hello-world/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/hello-world/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.comments" is captured twice
