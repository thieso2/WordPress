...
# spec-id: PT-S008
# traceability:
#   screen: web.category (SCR-0134)
#   legacy_origin: wp-includes/template-loader.php -> category
...
Feature: Visual parity for web.category
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.category" taken from the oracle
    When the target renders the route "/category/uncategorized/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/category/uncategorized/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.category" is captured twice
