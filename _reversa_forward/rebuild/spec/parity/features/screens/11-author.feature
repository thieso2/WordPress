...
# spec-id: PT-S011
# traceability:
#   screen: web.author (SCR-0137)
#   legacy_origin: wp-includes/template-loader.php -> author
...
Feature: Visual parity for web.author
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.author" taken from the oracle
    When the target renders the route "/author/admin/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/author/admin/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.author" is captured twice
