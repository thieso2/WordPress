...
# spec-id: PT-S006
# traceability:
#   screen: web.page (SCR-0132)
#   legacy_origin: wp-includes/template-loader.php -> page
...
Feature: Visual parity for web.page
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.page" taken from the oracle
    When the target renders the route "/sample-page/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/sample-page/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.page" is captured twice
