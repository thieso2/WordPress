...
# spec-id: PT-S010
# traceability:
#   screen: web.taxonomy (SCR-0136)
#   legacy_origin: wp-includes/template-loader.php -> taxonomy
...
Feature: Visual parity for web.taxonomy
  As the system
  ...
  Scenario: Rendered output matches the golden capture
    ...
    And a golden capture of "web.taxonomy" taken from the oracle
    When the target renders the route "/sample-tax/term/"
  ...
  Scenario: Design tokens resolve from the theme token system
    Given the target renders the route "/sample-tax/term/"
  ...
  Scenario: The capture is deterministic
    ...
    When "web.taxonomy" is captured twice
