...
## Mandatory linguistic patterns

| Field | Expected pattern |
|---|---|
| `type` | a premium adjective + the material + the product's name |
| `action` | an impactful verb + frozen motion + context |
| `surrounding_elements` | a visual noun + a motion/texture detail |
| `placement` | "centered hero [product] on [surface] with [reflection]" |
| `lighting.effects` | rim / key / top or back / an optional extra |
| `background` | the main color + the transition + an effect (bokeh, gradient...) |
| `rendering_style` | a realism or visual-style descriptor |
