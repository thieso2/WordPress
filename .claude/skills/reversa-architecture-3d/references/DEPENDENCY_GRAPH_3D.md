...
A directed graph of dependencies between modules, visualized in 3D with a force simulation (attraction between connected nodes, repulsion between unconnected ones) that distributes the graph organically through space.

## Mapeamento

| Code attribute | The node's visual attribute |
|---|---|
| Module | A sphere or an icon |
| Size (LOC or the number of dependents) | The sphere's radius |
| Module type | Color |
| Folder | A secondary color, or a nearby cluster |
| Edge = `imports/requires` | A directed curved line (with an arrow) |
| Edge weight (usage frequency) | Line thickness |

## When to use it

- Detecting **high coupling** (heavily connected nodes end up at the cluster's center).
- Identifying **central modules** (high fan-in) and **isolated modules**.
- Visualizing **dependency cycles** (loops visible in the simulation).
- Comparing cohesion between folders (modules from the same folder should end up close together).

**When to avoid it**: projects with more than ~300 modules, where the graph turns into an illegible hairball. Use Code City or group by folder.

## Layout algorithm: 3D force

A D3-force-style simulation adapted to 3 dimensions. It runs in a loop until it stabilizes, then freezes.
...
For large graphs (>200 nodes), replace the O(n²) repulsion with an **octree (Barnes-Hut)** to bring it down to O(n log n).

## Rendering the nodes

Use an `InstancedMesh` of spheres for up to 1,000 nodes; above that, billboards with sprites.
...
Curved lines in 3D using a `BufferGeometry` with `LineSegments`, or a `TubeGeometry` for edges with volume.
...
For directed edges with a visible arrow, use an `ArrowHelper` or small cones near the target.
...
Once it stabilizes, keep rendering but pause the simulation to save CPU.

## Cycle detection

Run Tarjan or Kosaraju before rendering; nodes belonging to a cycle get a special color (orange) and their edges turn red.
...
<aside id="sidebar">
    ...
    <label>
        <input type="checkbox" data-param="showLabels"> Labels visible
    </label>

    <button id="reset">Reset</button>
    <button id="freeze">Freeze the simulation</button>
    <button id="export-png">Exportar PNG</button>
</aside>
```

Changes to the sliders reactivate the simulation for another 100 frames before it freezes again.

## Interaction

- **Hovering a node**: a tooltip with its name, its number of dependents (fan-in) and its dependencies (fan-out).
- **Clicking a node**: highlights the node and its connected edges, and dims the rest (reduced opacity).
